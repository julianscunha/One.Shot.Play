...
const PROVIDERS = {
  ...
  openrouter: {
    ...
    defaultModel: process.env.OPENROUTER_TEXT_MODEL || 'google/gemini-3.5-flash',
    models: [
      'openai/gpt-5.5',
  ...
  async generateText(prompt, options = {}) {
    ...
    if (this.gemini) {
      try {
        ...
        if (!response?.text) {
          this.logger.error(
            `Gemini returned an empty response: ${JSON.stringify(response)}`
          );
        ...
        this.logger.error(
          `Gemini generation failed (${model}): ${error.message}`
        );
    ...
    try {
      ...
      if (!response?.choices?.length) {
        this.logger.error(
          `Invalid AI response from ${this.providerName}/${model}: ${JSON.stringify(response)}`
        );

        throw new Error(
          `AI provider returned no choices (provider: ${this.providerName}, model: ${model})`
        );
      }
      ...
      if (!content) {
        this.logger.error(
          `AI provider returned empty content: ${JSON.stringify(response)}`
        );

        throw new Error(
          `AI provider returned empty content (provider: ${this.providerName}, model: ${model})`
        );
      }
      ...
      this.logger.error(
        `AI generation failed (${this.providerName}/${model}): ${error.message}`
      );
