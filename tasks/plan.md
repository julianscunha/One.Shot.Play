# Implementation Plan: Refatoração Completa do YouTube Automation Agent

## Overview

Refatorar completamente o YouTube Automation Agent para uma aplicação web profissional rodando no Back4App free tier. A arquitetura será simplificada (único modo de inicialização), com interface web 100% funcional, multi-provider com fallback, pipeline de vídeo robusto com retry por fase, sistema de templates, Telegram bot, e otimizações de performance e segurança.
...
- **Back4App free tier:** Express + MongoDB (nativo), HTTPS incluso
- **Single entry point:** Apenas `node index.js`, sem modos
- **Setup via web:** Primeira execução redireciona para `/setup`
- **Multi-provider com fallback:** Cada serviço (AI, TTS, Image, Video) tem primary + fallback
- **Pipeline em fases:** Cada fase é independente, retryable e com checkpoint
- **Templates:** CRUD completo, snapshots preservados nos agendamentos
- **Segurança:** Helmet, rate limiting, API key, CORS restrito

## Task List

### Fase 0: Infraestrutura e Setup (Sprint 1)
**Objetivo:** Estrutura base, deploy no Back4App, setup inicial
...
**Checkpoint:** App sobe no Back4App, banco conecta, `/setup` acessível

**Limpeza pós-fase:**
- Remover `walkthrough.js`, `setup.js` interativo antigo, modos legados
- Remover dependências não usadas
- Remover arquivos de configuração legados (`credentials.json`, `.env.example` antigo)

---

### Fase 1: Sistema de Configuração e Templates (Sprint 1-2)
**Objetivo:** Interface de configuração completa, multi-provider com fallback, templates
...
**Checkpoint:** Configuração completa via web, templates funcionando, fallback testado
...
### Fase 2: Pipeline de Vídeo e Retry por Fase (Sprint 2-3)
**Objetivo:** Engine de execução robusta com retry por fase, verificação pré/pós
...
**Checkpoint:** Pipeline executa vídeo completo, retry por fase funciona, verificação pré/pós ativa, cancelamento funcional

**Limpeza pós-fase:**
- Remover agents antigos (`agents/*.js`) que foram substituídos por services
- Remover `database/db.js` legado (SQLite)
- Remover dependências SQLite/nedb
- Remover modos de inicialização antigos (`workflow`, `simple`)
...
### Fase 3: Interface Web e Dashboard (Sprint 3-4)
**Objetivo:** Dashboard profissional com dados reais, logs, métricas

- [ ] Task 3.1: Redesenhar layout do dashboard (estilo empresarial)
  - Acceptance: Dashboard com design limpo, profissional, em pt-BR
...
**Checkpoint:** Dashboard profissional com dados reais, logs, métricas, progress bar

**Limpeza pós-fase:**
- Remover código legado de dashboard antigo (`dashboard/index.html` mockado)
- Remover rotas e endpoints não utilizados

---

### Fase 4: Telegram Bot (Sprint 4)
**Objetivo:** Bot funcional com comandos, questionário interativo, notificações

- [ ] Task 4.1: Criar bot do Telegram e configurar webhook/polling
  - Acceptance: Bot responde a comandos em pt-BR
  - Verify: Enviar `/start` → bot responde em pt-BR
...
- [ ] Task 4.3: Implementar comandos básicos (`/status`, `/schedule`, `/cancel`, `/logs`)
  - Acceptance: Todos os comandos respondem com dados reais em pt-BR
  - Verify: Cada comando retorna informação correta em pt-BR
...
- [ ] Task 4.5: Implementar notificações de execução (sucesso/falha)
  ...
  - Verify: Executar publicação → bot envia report em pt-BR
...
- [ ] Task 4.6: Adicionar alertas de custo e indisponibilidade
  ...
  - Verify: Simular falha de serviço → alerta no Telegram em pt-BR
...
**Checkpoint:** Bot funcional com todos os comandos, questionário funciona, notificações enviadas
...
### Fase 5: Sistema de Custo e Métricas (Sprint 4-5)
**Objetivo:** Controle de orçamento, métricas integradas, análise de mercado
...
**Checkpoint:** Limite de custo funciona, alertas ativos, métricas do YouTube exibidas
...
### Fase 6: Otimização e Performance (Sprint 5)
**Objetivo:** Auditoria, cache, memória, otimização de recursos
...
**Checkpoint:** Código limpo, cache ativo, medição de melhoria documentada
...
### Fase 7: Segurança e Hardening (Sprint 5-6)
**Objetivo:** Preparar para produção pública
...
**Checkpoint:** App seguro para produção pública, todos os checks passam
...
### Fase 8: Testes e Validação Final (Sprint 6)
**Objetivo:** Garantir qualidade antes do deploy final
...
**Checkpoint:** Todos os testes passam, deploy validado, critérios de aceite atendidos

**Limpeza pós-fase:**
- Remover testes de funcionalidades removidas
- Remover mocks e fixtures não utilizados

**PonyTail (Fase 8/9 - Final):**
...
### Fase 9: Deploy e Documentação (Sprint 6)
**Objetivo:** Deploy final e documentação
...
- [ ] Task 9.2: Escrever README completo
  - Acceptance: README com setup, uso, arquitetura em pt-BR
...
- [ ] Task 9.4: Criar guia de uso para o usuário
  - Acceptance: Guia passo-a-passo para usar o sistema em pt-BR
...
**Checkpoint:** Deploy em produção, documentação completa, monitoramento ativo
...
- Fase 3 (Dashboard) pode começar após Fase 1 (Config)
- Fase 4 (Telegram) pode começar após Fase 2 (Pipeline)
...
|------|--------|------------|
| Back4App free tier tem limitações de workers | High | Usar servidor Express para agendamento, não workers separados |
| YouTube Data API quota esgotada | High | Cache de análises, limites por canal, priorizar conteúdo |
| OpenRouter free tier limitado | Medium | Cache agressivo, fallback para modelos menores, monitoramento de custo |
| Alucinação em análise de imagens | Medium | Usar múltiplos modelos de visão, consenso, threshold de confiança |
| Retry infinito em falhas transitórias | Medium | Backoff exponencial com limite, circuit breaker |

## PonyTail Schedule

| Momento | Fases | Ação |
|---------|-------|------|
| 1ª rodada | Fase 2 | Análise + aplicação de sugestões |
| 2ª rodada | Fase 4 | Análise + aplicação de sugestões |
| 3ª rodada | Fase 6 | Análise + aplicação de sugestões |
| 4ª rodada | Fase 8 | Análise + aplicação de sugestões finais |

## Open Questions

1. Back4App suporta workers agendados? Precisaremos de `node-cron` ou similar no servidor principal.
2. Qual modelo de visão usar para detecção de alucinações? GPT-4o, Claude Vision, ou outro?
3. YouTube Analytics API requer OAuth por canal. Como gerenciar refresh tokens para múltiplos canais?
4. Deploy contínuo: Back4App suporta GitHub Actions nativamente ou precisamos de script customizado?
